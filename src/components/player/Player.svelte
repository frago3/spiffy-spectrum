...
<div class="mt-auto flex gap-x-3">
    ...
    <div class="flex flex-col justify-end w-full">
        <div class="flex gap-x-2 flex-wrap items-center">
            <span class="font-bold md:hidden">{track.name}</span>
            <span class="hidden md:block">{albumName} - {track.name}</span>
            <span class="text-sm">{formatTime(currentTime)} / {formatTime(duration)}</span>
        </div>
        <div class="flex">
            <input class="accent-gray-900 cursor-pointer flex-grow mr-3" type="range" bind:value={currentTime} max={duration}/>
